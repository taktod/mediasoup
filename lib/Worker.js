...
console.log("ここでプロセスをつくろうとしてる。");
console.log(spawnArgs, spawnOptions);
		// これ・・・起動する必要ないんじゃね？
...
//		this._channel = new Channel(this._child.stdio[CHANNEL_FD]);
		this._channel = new Channel(null);
...
/*		this._child.stdout.on('data', (buffer) =>
		{
			for (let line of buffer.toString('utf8').split('\n'))
			{
				if (line)
	...
	createRoom(options)
	{
		...
		return this._channel.request('worker.createRoom', internal, options)
			.then((data) =>
			{
				...
				let room = new Room(internal, data, this._channel);
