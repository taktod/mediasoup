...
const ms = require("bindings")("mediasoupGyp");
// これは別にいつ発動してもいいのかな
// ここで発動してはいけない。
//ms.init();
				...
				if (!this._lastBinaryNotification)
				{
					try
					{
						...
						switch (nsPayload[0])
						{
							...
							default:
								workerLogger.error('unexpected data: %s', nsPayload.toString('utf8'));
						}
					}
					catch (error)
					{
...
*/
	...
	close()
	{
		...
		for (let sent of this._pendingSent.values())
		{
			sent.close();
		}
...
/*		this._socket.removeAllListeners('end');
	...
	request(method, internal, data)
	{
		...
		// おくった応答がこないから、promiseまでこないのね。
		var promise = new Promise((pResolve, pReject) =>
		{
			let sent =
			{
				resolve: (data) =>
...
console.log("promiseで応答の処理設定した。");
			// Add sent stuff to the Map.
			this._pendingSent.set(id, sent);
		});
		try
		{
			// この段階で処理して、応答がかえってくるのか・・・
			ms.request(ns);
